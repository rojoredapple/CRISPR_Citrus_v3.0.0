...
//utils.$document.ready(function () {}) # I've changed because it would only work after a restart. With addEventListener it works all the time.
  ...
  document.addEventListener("turbolinks:load", function () {
    var $navbar = $('.navbar-theme-mobile');
  
    if ($navbar.length) {
      var windowHeight = utils.$window.height();
      utils.$window.scroll(function () {
        var scrollTop = utils.$window.scrollTop();
        var alpha = (scrollTop + 200) / windowHeight * 2;
        alpha >= 1 && (alpha = 1);
        $navbar.css({
          'background-color': "rgba(11, 23, 39, " + alpha + ")"
        });
